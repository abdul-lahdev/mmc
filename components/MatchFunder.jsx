...
export default function MatchFunder() {
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 px-3 space-y-6">
            {/* Stepper */}
            <div className="flex items-center space-x-4 justify-center">
                <div className='flex flex-col  items-center gap-2'>
                    <div className={`w-[40px] h-[40px] ${step >= 1 ? 'bg-[#2B7FFF]' : 'bg-[#2B7FFF]'} rounded-[50%] flex items-center justify-center`}>
                        <div className='w-[32px] h-[32px] bg-[#fff] rounded-[50%] flex items-center justify-center'>
                    ...
                    <p className='text-[10px] text-[#020618] font-[400]'>Match Funders</p>
                </div>
                <div className="w-10 border-t-2 mb-[25px] border-[#2B7FFF]"></div>
                <div className='flex flex-col  items-center gap-2'>
                    ...
                    <p className={`text-[10px] ${step >= 2 ? 'text-[#020618]' : 'text-[#45556C]'} font-[400]`}>Select Template</p>
                </div>
                <div className={`w-10 border-t-2 mb-[25px]  ${step >= 2 ? 'border-[#2B7FFF]' : 'border-[#F2F4F7]'}`}></div>
                <div className='flex flex-col items-center gap-2'>
                    ...
                    <p className={`text-[10px] ${step >= 3 ? 'text-[#020618]' : 'text-[#45556C]'} font-[400]`}>Send Offer</p>
                </div>
            ...
            {step === 1 && (
                <div>
                    <div className='relative'>
                        <div className='absolute top-0 translate-x-2 translate-y-2'>
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg"> <path d="M17.5 17.5L12.5001 12.5M14.1667 8.33333C14.1667 11.555 11.555 14.1667 8.33333 14.1667C5.11167 14.1667 2.5 11.555 2.5 8.33333C2.5 5.11167 5.11167 2.5 8.33333 2.5C11.555 2.5 14.1667 5.11167 14.1667 8.33333Z" stroke="#62748E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /> </svg>
                        </div>
                    </div>
                    <input className='mb-3 bg-[#F8FAFC] rounded-[6px] h-[40px] border border-[transparent] w-full pl-8 focus-visible:outline-[transparent]' placeholder='Search' />
                    <h2 className="text-[#314158] text-[16px] font-[500] mb-4">Suggested</h2>
                    <div className="space-y-2">
                        {funders.map(funder => (
                            <div key={funder.id} className="flex items-center justify-between border p-4 rounded-[8px]">
                                <label className="flex items-center space-x-2">
                                    ...
                                    />
                                    <div className='w-[32px] h-[32px] rounded-[50%] flex items-center justify-center text-[#90A1B9] text-[12px] font-[500] bg-[#F1F5F9]'>ss</div>
                                    <div>
                                        <p className="text-[#0F172B] text-[14px] font-[500]">{funder.name}</p>
                                        <p className="text-[#45556C] text-[12px] font-[400]">{funder.locations}</p>
                                    </div>
                                </label>
                                <p className="text-[#0F172B] text-[14px] font-[500]">{funder.amount}</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {step === 2 && (
                <div>
                    ...
                    <input className='mb-3 bg-[#F8FAFC] rounded-[6px] h-[40px] border border-[transparent] w-full pl-8 focus-visible:outline-[transparent]' placeholder='Search' />
                    <h2 className="text-[#314158] text-[16px] font-[500] mb-3">Template</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {templates.map((template, index) => (
                            ...
                            >
                                <p className="text-[#0F172B] text-[14px] font-[500]">{template.title}</p>
                                <p className="text-[#45556C] text-[12px] font-[400] mt-1 whitespace-pre-line">{template.body.substring(0, 120)}...</p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-4">
                <button
                    className="w-full border border-[#F1F5F9] bg-[#F8FAFC] rounded-[60px] h-[38px] text-[12px] text-[#62748E] font-[500] hover:bg-[#e6e6e6]"
                    onClick={() => setStep(prev => Math.max(1, prev - 1))}
                >
                    Cancel
                </button>
                <button
                    className="w-full h-[38px] rounded-[60px] border-[2px] border-[#8EC5FF] bg-[#2B7FFF] text-[#fff] text-[12px] font-[700] hover:bg-[#2572e7]"
